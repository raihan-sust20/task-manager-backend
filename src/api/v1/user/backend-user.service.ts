...
import { PoolService } from '../pool/pool.interface';
/**
...
export class UserService implements OnModuleInit {
  ...
  constructor(
    ...
    @Inject('PHALA_POOL_SERVICE_GRPC')
    private poolGRPCClient: ClientGrpc,
  ) {}

  onModuleInit(): void {
    this.authService = this.grpcClient.getService<AuthService>('AuthService');
    this.poolService =
      this.poolGRPCClient.getService<PoolService>('PoolService');
  }
  ...
  // TODO: remove role from signininput it was a mistake
  ...
  private formatSignupResponse = (
    response: ISignupResponseData,
  ): SignupResponseType => {
    const {
      userId,
      email,
  ...
  async changeUserEmail(
    ...
    try {
      ...
      const updatePoolUserRequest = new naas.pool.user.UpdateUsersRequest({
        data: [
          {
            whereQuery: { userId },
            data: { email: newEmail },
      ...
      await this.poolService.updateUsers(updatePoolUserRequest).toPromise();
    } catch (error) {
      this.logger.warn(
        'Something wrong when update user email in phala pool service. Please check the database',
      );
  ...
  async changeUserPassword(
    ...
    const request = new naas.auth.ChangeUserPasswordRequest(
    ...
    const { ChangeUserPasswordResponseStatus } =
      naas.auth.ChangeUserPasswordResponse;
  ...
  async validatePassword(
    ...
    const request = new naas.auth.ValidatePasswordRequest(
    ...
    const { ValidatePasswordResponseStatus } =
      naas.auth.ValidatePasswordResponse;
  ...
  async sendForgotPasswordEmail(
    ...
    const request = new naas.auth.SendForgotPasswordEmailRequest(
    ...
    const { SendForgotPasswordEmailResponseStatus } =
      naas.auth.SendForgotPasswordEmailResponse;
  ...
  async validateForgotPassword(
    ...
    const request = new naas.auth.ValidateForgotPasswordRequest(
    ...
    const { ValidateForgotPasswordResponseStatus } =
      naas.auth.ValidateForgotPasswordResponse;
  ...
  async resendActivation(
    ...
    const request = new naas.auth.ResendActivationRequest(
    ...
    const { ResendActivationResponseStatus } =
      naas.auth.ResendActivationResponse;
  ...
  async validatePromotionCode(
    ...
    const request = new naas.auth.stripe.ValidatePromotionCodeRequest({
      promotionCode,
    });
